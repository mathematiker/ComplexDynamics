...
Das Newton-Verfahren konvergiert lokal. Wie ist das Konvergenzverhalten außerhalb dieser Konvergenzumgebung? In Abbildung 1 wurde das Konvergenzverhalten für gegebene Startwerte durch unterschiedliche Farben hervorgehebt. Vergleichen wir zwei benachbarten Werte auf ihre Dynamik so stellen wir fest, dass es Punkte gibt in denen in keiner Umgebung alle Punkte diesselbe Dynamik besitzen.
...
Ist $z_0$ ein anziehender periodischer Punkt von $f$ mit Periode $n$, dann ist die Menge
\[
	A_f(z_0)=\{z\in \overline{\mathbb{C}}: \lim\limits_{k\to \infty} f^{k\cdot n}(z)=z_0\}
\]
...
\textbf{Fall 1: $0<|a|<1$.} so konvergiert für jede kompakte Teilmenge die Funktionenfolge $F^n(z)=a^n\cdot z$ gleichmäßig gegen 0. In normal auf jeder Umgebung $U\subset \C$. 
...
\begin{prop} \label{13}
Sei $z_0$ ein anziehender periodischer Punkt bzgl. einer analytischen Funktion $f$, so existiert eine (offene) Umgebung $U$ um $z_0$, sodass $U\subset A_f(z_0)$. Wir nennen die Zusammenhangskomponente von $z_0\in A_f(z_0)$ auch das \emph{unmittelbare Einzugsgebiet} (engl. \emph{immediate basin of attraction}) von $z_0$ bzgl. $f$.
\end{prop}
...
\begin{ex}
Sei $P$ ein Polynom vom Grad $n\ge 2$, dann ist $\infty$ ein superanziehender Fixpunkt bzgl. $P$. Tatsächlich ist 
\[
...
Sei P ein Polynom vom Grad $n\ge 2$ und sei $z_0$ ein (super-)anziehender periodischer Punkt von $P$. Dann liegt im Einzugsgebiet von $z_0$ bzgl. $P$ ein kritischer Punkt.
\end{thm}
...
Dadurch erhalten wir eine analytische Abbildung $F^{-1}: B_{r^*}(0) \to U$, die surjektiv ist. Diese ist mit dem Monodromielemma eindeutig bestimmt. Es bleibt zu zeigen, dass $F^{-1}$ injektiv ist. Angenommen $F^{-1}(z_1)=F^{-1}(z_2)$. Dann ist $F(F^{-1}(z_1))=F(F^{-1}(z_2))$ und da $F^{-1}$ gerade lokal die Umkehrabbildung beschreibt auch $z_1=z_2$.
